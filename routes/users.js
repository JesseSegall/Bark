...
router.post("/searchSitter", async (req, res) => {
  try {
    ...
    const searchTermFull = req.body.search_term
      .toLowerCase()
      .replace(/\s/g, "");
    ...
    for (i = 0; i < sitterList.length; i++) {
      for (j = 0; j < currentUser.length; j++) {
        if (
          sitterList[i].firstName.toLowerCase() +
            sitterList[i].lastName.toLowerCase() ==
...
router.post("/requestSitter", async (req, res) => {
  ...
  try {
    newRequest = await requests.addRequest(
      ownerID,
      sitterId,
      dogId,
      requestText
